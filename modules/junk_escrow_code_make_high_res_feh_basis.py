def transforms_lit_feh():
    '''
    Modify some of the datasets on an individual basis
    '''
    ...
    df_lambert_logeps["feh"] = np.subtract(df_lambert_logeps["log_eps_fe_spec"], 7.51)

    # average the values in Chadid from FeI and FeII lines
    df_chadid_feh["feh"] = np.mean([df_chadid_feh["fehI"].values,
                                      df_chadid_feh["fehII"].values], axis=0)

    # Kolenberg+ 2010 only has a single data point (ref. Chadid+ 2017, Fig. 7)
